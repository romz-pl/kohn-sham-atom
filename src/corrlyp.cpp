...
const double CorrLyp::m_cf = (3. / 10) * pow(3 * RATOM_PI * RATOM_PI, 2. / 3.);
...
{
    // It is sufficient to change the arguments in function A()
    return A(f, delta, rhob, rhoa);
}

/*
//
// Returns value of function $\omega(rho)$
//
...
//
// Returns value of function $\omega(rho)$ for x = rho^{-1/3}
//
double CorrLyp::OmegaT(double x) const
{
// funct calculation of function pow(x, 11);
double y = x * x; // x^2
...
//
// Returns value of function $\Tilde{\omega}'(x)$
//
...
/*
//
// Returns value of function $\delta(rho)$
//
...
//
// Returns value of function $\delta(rho)$ for x = rho^{-1/3}
//
...
//
// Returns value of function $\Tilde{\delta}'(x)$
//
...
//
// Returns value of partial derivative for finctional of correlation density.
// Partial derivative is for $\rho_{\alpha}$
//
...
//
// Returns value of partial derivative for finctional of correlation density.
// Partial derivative is for $\rho_{\beta}$
//
...
//
// Partial derivative $\frac{\partial A(\rho_{\alpha}, \rho_{\beta})}{\partial \rho_{\alpha}}$
//
...
//
// Partial derivative $\frac{\partial B(\rho_{\alpha}, \rho_{\beta})}{\partial \rho_{\alpha}}$
//
...
//
// Partial derivative $\frac{\partial C(\rho_{\alpha}, \rho_{\beta})}{\partial \rho_{\alpha}}$
//
